...
import Refusal from "./modals/Refusal";
import NewActivity from "./modals/NewActivity";
import LaunchActivity from "./modals/LaunchActivity";
                ...
                {type === "suivi" ? (
                  ...
                  activity.status === "Validé" ? (
                    ...
                    <td style={{ fontStyle: "italic" }}>
                      {activity.status} ({activity.currentVotes} votes)
                    </td>
                  ) : (
                    <FormButton
...
export function StudentTable({ data = [] }) {
  ...
  const openModal = (name, reason) =>
    setModalState({ isOpen: true, name, reason });
  const closeModal = () => setModalState({ isOpen: false });

  const handleReadAbsence = (student) => () =>
    openModal(student.name, student.reason);
        ...
        <tbody className="table-body">
          {data.map((student) => (
            <tr className="line" key={student.id}>
              ...
              <td>
                <FormButton
                  onClick={handleReadAbsence(student)}
                  text={"Voir motif"}
                />
              </td>
            </tr>
